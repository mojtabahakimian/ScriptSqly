...
BEGIN
    ...
    -- ضرایب نسبت‌گیری اقلام ماهیانهٔ واردشده از صفحه کارکرد (پاداش/حق ناقل).
    -- @MRATE_D از کارکرد اسمی (DAYS) و @MRATE_DB از کارکرد رسمی (DAYSB) ساخته می‌شود.
    DECLARE @MRATE_D DECIMAL(18,6), @MRATE_DB DECIMAL(18,6);
    ...
    BEGIN
        ...
        -- پاداش و حق ناقل به‌صورت «مبلغ کلی ماهانه» در صفحه کارکرد وارد می‌شوند، پس با
        -- روشن‌بودن MONTHLY_ITEM_PRORATE باید مثل بقیه اقلام ماهیانه نسبت‌گیری شوند:
        -- مخرج ثابت ۳۰ روز، آستانه «۲۸ روز و بیشتر = ماه کامل»، و دو ریل جدا طبق
        -- PAY_BASE_DAYS (پرداختی) و INS_BASE_DAYS (بیمه/مالیات) خود آیتم.
        -- برخلاف اقلام حکم، اینجا PRORATE_FACTOR اعمال نمی‌شود؛ این مبالغ به کل ماه
        -- تعلق دارند و به بازه اعتبار حکم گره نخورده‌اند.
        SET @MRATE_D  = CASE WHEN @MONTHLY_PRORATE = 0 OR @DAYS  >= 28 THEN 1.0 ELSE @DAYS  / 30.0 END;
        SET @MRATE_DB = CASE WHEN @MONTHLY_PRORATE = 0 OR @DAYSB >= 28 THEN 1.0 ELSE @DAYSB / 30.0 END;

        IF @PERF_AMOUNT > 0
            INSERT INTO @ItemCalc (ITEM_ID, ITEM_CODE, ITEM_TYPE, AMOUNT, INS_AMOUNT, INS_SUBJECT, TAX_SUBJECT)
            SELECT ITEM_ID, 'PERF_BONUS', 2,
                   CAST(@PERF_AMOUNT * (CASE WHEN CALC_BASIS <> 2 THEN 1.0 WHEN PAY_BASE_DAYS = 1 THEN @MRATE_D ELSE @MRATE_DB END) AS BIGINT),
                   CAST(@PERF_AMOUNT * (CASE WHEN CALC_BASIS <> 2 THEN 1.0 WHEN INS_BASE_DAYS = 1 THEN @MRATE_D ELSE @MRATE_DB END) AS BIGINT),
                   INS_SUBJECT, TAX_SUBJECT FROM #ItemDefSource WHERE ITEM_CODE = 'PERF_BONUS';

        IF @TRANSP_AMOUNT > 0
            INSERT INTO @ItemCalc (ITEM_ID, ITEM_CODE, ITEM_TYPE, AMOUNT, INS_AMOUNT, INS_SUBJECT, TAX_SUBJECT)
            SELECT ITEM_ID, 'TRANSP', 2,
                   CAST(@TRANSP_AMOUNT * (CASE WHEN CALC_BASIS <> 2 THEN 1.0 WHEN PAY_BASE_DAYS = 1 THEN @MRATE_D ELSE @MRATE_DB END) AS BIGINT),
                   CAST(@TRANSP_AMOUNT * (CASE WHEN CALC_BASIS <> 2 THEN 1.0 WHEN INS_BASE_DAYS = 1 THEN @MRATE_D ELSE @MRATE_DB END) AS BIGINT),
                   INS_SUBJECT, TAX_SUBJECT FROM #ItemDefSource WHERE ITEM_CODE = 'TRANSP';
